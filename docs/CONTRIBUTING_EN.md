Contributing to Lightingluminol
=======================
...
Lightingluminol uses as the same patching system as Paper,  
and has been divided into two directories for the purpose of modifying different parts of it:

- `lightingluminol-api` - Modifications to `Folia-API` / `Paper-API` / `Spigot-API` / `Bukkit-API`.
- `lightingluminol-server` - Modifications to Minecraft Vanilla Server's source logic.
...
3. After performing the operation, the following directory pairs should exist in the root directory of the warehouse: `lightingluminol-api` and `lightingluminol-server` , `luminol-api` and `luminol-server` , `folia-api` and `folia-server` , `paper-api` and `paper-server` (Referred to `*-api` and `*-server` as below)
...
- `lightingluminol-api` : Modifications to the new API
...
- `lightingluminol-server` : Changes and new files to the Minecraft vanilla server should be made in this folder
...
- Only commits after the last commit of Luminol will be considered as Lightingluminol patches.
...
2. Run `git commit -a --fixup <hash>` in your terminal to make a fix-up commit (PS: do not commit changes of lightingluminol-created files)
...
4. Run Gradle's task `fixupPaperApiFilePatches` to regenerate lightingluminol-created files to patches (PS: do not commit again before you run this task)
